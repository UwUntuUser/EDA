...
void funcionarios(int etapa, tDatos const& d, std::vector<int>&sol, int tiempoActual, std::vector<bool> &marcas, int &tiempoMejor, std::vector<int>const& acum)
{
	for (int i = 0; i < d.numTareas; i++)
	{
		...
		if (!marcas[i])
		{
			...
			else if (tiempoActual + acum[etapa + 1] < tiempoMejor)
			{
				funcionarios(etapa + 1, d, sol, tiempoActual, marcas, tiempoMejor, acum);
			}
			marcas[i] = false;
		}
		tiempoActual -= d.tiempos[etapa][i];
...
bool resuelveCaso() {
	...
	for (int i = 0; i < datos.numFuncionarios; i++)
		tiempoMejor += datos.tiempos[i][i];  //solucion de tiempo inicial con la diagonal

	std::vector<int>acum(datos.numFuncionarios, 10000); // crear vector de acumulados

	for (int i = 0; i < datos.numFuncionarios; i++)
		for (int j = 0; j < datos.numFuncionarios; j++)
			acum[i] = std::min(datos.tiempos[i][j], acum[i]);

	for (int i = datos.numFuncionarios - 1; i > 0; i--)
		acum[i - 1] += acum[i];

	funcionarios(0, datos, sol, 0, marcas, tiempoMejor,acum);
